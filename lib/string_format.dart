class StringFormat {
	...
	static String classMethod(String string) =>
		StringFormat.lcFirst(StringFormat.titleCase(string));

	static String pluralClassMethod(String string) => StringFormat.lcFirst(
		StringFormat.plural(StringFormat.titleCase(string)));

	static String classProperty(String string) =>
		StringFormat.lcFirst(StringFormat.titleCase(string));

	static String pluralClassProperty(String string) =>
		StringFormat.lcFirst(
			StringFormat.plural(StringFormat.titleCase(string)));

	static String url(String string) =>
		StringFormat.variable(string).replaceAll('_', '-');

	static String pluralUrl(String string) =>
		StringFormat.url(StringFormat.pluralVariable(string));

	static String variable(String string) =>
		StringFormat.getWords(string, true).join('_');

	static String constant(String string) {
		String result = StringFormat.variable(string).toUpperCase();
		if (result == string) {
			return "${result}_COLUMN";
		}
		return result;
	}

	static String pluralVariable(String string) =>
		StringFormat.plural(StringFormat.variable(string));

	static String titleCase(String string, [String delimiter = '']) =>
		StringFormat.getWords(string, true).map((String t) =>
			StringFormat.ucFirst(t)).join(delimiter);

	static List<String> getWords(String string, [bool lowercase = false]) {
		String t = StringFormat.clean(string, ' ');
		t = t.replaceAllMapped(
			new RegExp('(.)([A-Z])([a-z])'), (match) => '${match.group(
			1)} ${match.group(2)}${match.group(3)}');
		t = t.replaceAllMapped(
			new RegExp('([a-z])([A-Z])'), (match) => '${match.group(1)} ${match
			.group(2)}');
		t = t.replaceAllMapped(
			new RegExp('([0-9])([^0-9])'), (match) => '${match.group(1)} ${match
			.group(2)}');
		if (lowercase) {
			t = t.toLowerCase();
		}
	...
	static String plural(String string) {
		Map<RegExp, String> plural = {
			...
			new RegExp(
				r'(matr|vert|ind)ix|ex$', caseSensitive: false): r"$1ices",
			...
			new RegExp(
				r'(?:([^f])fe|([lr])f)$', caseSensitive: false): r"$1$2ves",
			...
			new RegExp(
				r'(alias|status|campus)$', caseSensitive: false): r"$1es",
		...
		for (RegExp pattern in plural.keys) {
			if (pattern.hasMatch(word)) {
				...
				for (Match match in pattern.allMatches(word)) {
					replaced =
						replaced.replaceAll("\$${ind}", match.group(ind));
					++ind;
				}
